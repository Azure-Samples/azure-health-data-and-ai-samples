...
export async function getAppConsentInfo(clientId: string, scopes: string): Promise<AppConsentInfo> {
  ...
  try
  {
    ...
    throw Error(`Fatal error while accessing Backend Application Consent API. Does the auth function app have access to Azure Active Directory? Check your application settings: ${error}`);
  }
