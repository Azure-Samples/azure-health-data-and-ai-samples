...
public class SmartController : Controller
{
    ...
    public async Task<IActionResult> UserContextCallback(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(token.IdToken))
                HttpContext.Session.SetString(SessionKeys.UserContextIdToken, token.IdToken);
            else
                HttpContext.Session.Remove(SessionKeys.UserContextIdToken);
            HttpContext.Session.SetString(SessionKeys.LastSuccess,
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Single source of truth: IdpType selects both the token and the user-id claim.
        //   EntraId    -> id_token     + "oid" claim (stable Entra object id, identical in id/access tokens).
        //   ExternalIdp -> access_token + "sub" claim (Okta custom auth servers emit different `sub` values
        //                  for id vs access tokens, so id_token would never match the proxy's lookup).
        // The proxy uses the same IdpType to derive the same claim, so the cache key round-trips.
        var isEntra = string.Equals(
            (_configuration["SmartOnFhir:IdpType"] ?? "EntraId").Trim(),
            "EntraId",
            StringComparison.OrdinalIgnoreCase);

        var sessionKey = isEntra ? SessionKeys.UserContextIdToken : SessionKeys.UserContextToken;
        var claimType  = isEntra ? "oid" : "sub";
        var tokenForCache = HttpContext.Session.GetString(sessionKey);

        if (string.IsNullOrEmpty(tokenForCache))
        {
            HttpContext.Session.SetString(SessionKeys.LastError,
        ...
        // Extract user identifier using the per-IdP claim type. Same value the proxy will read
        // from the EHR access token. ReadJwtToken does NOT apply InboundClaimTypeMap.
        string? userId;
        try
        {
            var handler = new JwtSecurityTokenHandler();
            var jwt     = handler.ReadJwtToken(tokenForCache);
            userId = jwt.Claims.FirstOrDefault(c => c.Type == claimType)?.Value;
        }
        catch
        {
            HttpContext.Session.SetString(SessionKeys.LastError,
                "Could not read the current user context token to extract user identity.");
            return RedirectToAction("Index", "Home");
        }
        ...
        {
            HttpContext.Session.SetString(SessionKeys.LastError,
                $"User context token does not contain the '{claimType}' claim required for IdpType={(isEntra ? "EntraId" : "ExternalIdp")}.");
            return RedirectToAction("Index", "Home");
        }
        ...
        req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", tokenForCache);
