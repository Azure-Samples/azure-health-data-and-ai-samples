...
namespace SMARTCustomOperations.AzureAuth.Services
{
    public class GraphConsentService : BaseContextAppService
    {
        ...
        {
           if (string.IsNullOrEmpty(configuration.KeyVaultStore))
			{
				throw new ConfigurationErrorsException("KeyVaultStore must be set to use the KeyVaultClientConfiguratinService.");
			}
