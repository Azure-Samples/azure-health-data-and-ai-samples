...
public class SmartController : Controller
{
    ...
    {
        HttpContext.Session.SetString(SessionKeys.LaunchType, Infrastructure.LaunchType.Ehr);
        // Use lightweight User Context tokens (openid-only login), not SMART/FHIR tokens.
        // Prefer id_token for Entra context-cache validation; fall back to access_token for compatibility.
        var userContextAccessToken = HttpContext.Session.GetString(SessionKeys.UserContextToken);
        var userContextIdToken = HttpContext.Session.GetString(SessionKeys.UserContextIdToken);
        var tokenForCache = !string.IsNullOrWhiteSpace(userContextIdToken)
            ? userContextIdToken
        ...
        {
            HttpContext.Session.SetString(SessionKeys.LastError,
                "User context token does not contain the configured user id claim (SmartOnFhir:UserIdClaimType).");
            return RedirectToAction("Index", "Home");
        }
