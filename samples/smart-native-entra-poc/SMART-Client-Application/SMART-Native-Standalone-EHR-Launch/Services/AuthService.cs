...
public class AuthService
{
    ...
    public async Task<(string redirectUrl, string state, string codeVerifier)> BuildAuthorizationRequestAsync(
        ...
        string? iss = null)
    {
        // Always discover via this app's configured SMART proxy endpoint so auth
        // goes through /auth/authorize (where SMART->Entra scope translation happens).
        ...
        // For Entra, always request SMART scopes against the configured FHIR audience.
        var aud = _configuration["SmartOnFhir:FhirAudience"];
    ...
    /// Uses openid + profile scopes — no FHIR audience, no SMART scopes.
