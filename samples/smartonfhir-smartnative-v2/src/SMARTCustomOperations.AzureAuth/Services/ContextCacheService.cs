...
namespace SMARTCustomOperations.AzureAuth.Services
{
    public class ContextCacheService
    {
        ...
        public async Task<ClaimsPrincipal> ValidateContextAccessTokenAsync(string token)
        {
            ...
            {
                // External IdP path: the access_token sent by the User Context client
                // typically carries aud = FHIR audience (Okta custom auth servers default
                // to the configured resource). Some IdPs may instead emit the User Context
                // app's client id. Accept either to keep the contract operator-friendly.
                var validAudiences = new List<string>();
                if (!string.IsNullOrEmpty(_config.ContextAppClientId))
                {
                    validAudiences.Add(_config.ContextAppClientId);
                ...
                {
                    validAudiences.Add(_config.FhirAudience);
                    // FhirAudience may be stored with a trailing slash; also accept the
                    // un-slashed form because tokens commonly omit it.
                    var trimmed = _config.FhirAudience.TrimEnd('/');
                    if (!string.Equals(trimmed, _config.FhirAudience, StringComparison.Ordinal))
                    {
                        validAudiences.Add(trimmed);
                    }
                }
