...
namespace SMARTCustomOperations.AzureAuth.Filters
{
    ...
    public sealed class ContextCacheInputFilter : IInputFilter
    {
        ...
        public ContextCacheInputFilter(ILogger<ContextCacheInputFilter> logger, AzureAuthOperationsConfig configuration, IIdpStrategy idpStrategy, ContextCacheService? cacheService = null)
        {
            _logger = logger;
        ...
        public string Name => nameof(ContextCacheInputFilter);
        ...
        public async Task<OperationContext> ExecuteAsync(OperationContext context)
        {
            ...
            {
                _logger?.LogError("Context cache endpoint called but CacheConnectionString is not configured.");
                FilterErrorEventArgs cacheError = new(name: Name, id: Id, fatal: true, error: new InvalidOperationException("Cache is not configured. Set CacheConnectionString to enable EHR launch context."), code: HttpStatusCode.ServiceUnavailable);
                OnFilterError?.Invoke(this, cacheError);
            ...
            {
                _logger?.LogWarning("User attempted to access context cache without a valid token. {Exception}", ex);
            ...
            string userIdClaimType = _idpStrategy.UserIdClaimType;
            if (userPrincipal is null || !userPrincipal.HasClaim(x => x.Type == userIdClaimType))
            {
                _logger?.LogError("User does not have the {ClaimType} claim. {User}", userIdClaimType, userPrincipal);
                FilterErrorEventArgs error = new(name: Name, id: Id, fatal: true, error: new UnauthorizedAccessException("Token validation failed for context cache operation"), code: HttpStatusCode.Unauthorized);
                OnFilterError?.Invoke(this, error);
                return context.SetContextErrorBody(error, _configuration.Debug);
            }
            ...
            var userId = userPrincipal.FindFirst(userIdClaimType)!.Value;
            if (launchCacheObject.UserId != userId)
            {
                _logger?.LogError("User provided id {ProvidedId} does not match token id {TokenId}.", launchCacheObject.UserId, userId);
                FilterErrorEventArgs error = new(name: Name, id: Id, fatal: true, error: new UnauthorizedAccessException($"User provided id {launchCacheObject.UserId} does not match token id {userId}."), code: HttpStatusCode.Unauthorized);
                OnFilterError?.Invoke(this, error);
                return context.SetContextErrorBody(error, _configuration.Debug);
            }
            ...
            {
                _logger?.LogError("Error while setting launch cache object. {Exception}", ex);
                throw;
            }
